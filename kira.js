...
if (isLimit(sender, isPremium, isOwner, limitCount, _limit)) return reply(mess.limit)
...
if (!isRegister) return reply(mess.regist)
checkLimit(sender)
sendButMessage(from, `CLICK DIBAWAH UNTUK MEMBELI LIMIT`, [
            {              
...
limitawal = args[0]
reply(`Limit berhasil di ubah menjadi : ${settings.limitCount}`)
break
case 'resetlimit':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
reply(`WAIT BOS`)
...
case 'nsfw':
			...
			if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					...
					break
case 'fb':
return reply ('sorry,this feature under maintenance')
break
				case 'kick':
			if (!isGroup) return reply(mess.only.group)
			if (!isGroupAdmins) return reply(mess.only.admin)
